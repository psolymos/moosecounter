...
updateCompModelTab <- function() {
  CompModelTab <- data.frame(AIC=sapply(CompModelList, AIC))
...
MooseCompSimMMU.PI1 <-
function(total_model_id, comp_model_id, do_avg=FALSE)
{
  opts <- getOption("moose_options")
  x <- MooseData
  ...
  if (!all(total_model_id %in% names(ModelList)))
  ...
  if (!(comp_model_id %in% names(CompModelList)))
    stop(paste(comp_model_id, " model cannot be found"))
  Mult.model <- CompModelList[[comp_model_id]]

...
mc_check_comp <- function(x) {
  ...
  if (opts$response == "total") {
    ...
    check <- (x_srv$MOOSE_TOTA - x_srv$UNKNOWN_AG) -
      (x_srv$BULL_LARGE +
        x_srv$BULL_SMALL +
        x_srv$ADULT_COW +
        x_srv$TOT_CALVES)
...
mc_fit_comp_formula <- function(formula, x) {
  ...
  for (i in seq_len(ncol(ymat) - 1L)) {
    ...
    names(m@coefficients)[j] <- gsub(
      paste0(":", i),
      paste0(":", colnames(ymat)[i]),
      names(m@coefficients)[j]
    )
  }
...
mc_predict_comp <- function(
  ...
  Survey.data$TOT_CALVES <- Survey.data$COW_1C +
    2 * Survey.data$COW_2C +
    Survey.data$LONE_CALF
  Unsurvey.data <- x[!srv, ]
  ...
  while (b <= B) {
    ...
    if (max(BSurvey.data$MOOSE_TOTA) != 0) {
      # loop 1
        ...
        if (max(predict.BNS, predict.BNSout) <= MAXCELL && CONVERGED) {
          ...
          if (newPI) {
            ...
            if (DUAL && inherits(total_fit, "wzi")) {
              Bm.NSout <- stats::predict(
                model.Boot$unweighted_model,
                newdata = Unsurvey.data[!Unsurvey.data$area_srv, ],
                type = "count"
              )
              ...
              if (inherits(total_fit, "hurdle")) {
                ...
                PUnsurvey.data[!Unsurvey.data$area_srv] <- rHurdle(
                  sum(!Unsurvey.data$area_srv),
                  mu.nb = Bm.NSout,
                ...
                PUnsurvey.data[!Unsurvey.data$area_srv] <- rZINB(
                  sum(!Unsurvey.data$area_srv),
                  mu.nb = Bm.NSout,
                  theta.nb = Btheta.nbout,
                  phi.zi = Bphi.ziout
          ...
          if (max(PUnsurvey.data) <= MAXCELL) {
            ...
            if (NS1 > 0) {
              ...
              for (i in 1:NS1) {
                pred.numbers1[i, ] <- stats::rmultinom(
                  1,
                  newdata1$MOOSE_TOTA[i],
                  pred.prob[i, ]
                )
              }
              ...
              UNKwhere <- which(
                Survey.data$MOOSE_TOTA > 0 & Survey.data$UNKNOWN_AG > 0
              )
              unk_df <- Survey.data[UNKwhere, , drop = FALSE]
              UNK1 <- nrow(unk_df)
              if (UNK1 > 0) {
                pred.prob.unk <- VGAM::predict(
                  Mult.model,
                  newdata = unk_df,
                  type = "response"
                )
                ...
                if (opts$response == "total") {
                  Survey.data1$BULL_LARGE[UNKwhere] <- Survey.data$BULL_LARGE[
                    UNKwhere
                  ] +
                    pred.unk1[, 1]
                  Survey.data1$BULL_SMALL[UNKwhere] <- Survey.data$BULL_SMALL[
                    UNKwhere
                  ] +
                    pred.unk1[, 2]
                  Survey.data1$COW_1C[UNKwhere] <- Survey.data$COW_1C[
                    UNKwhere
                  ] +
                    pred.unk1[, 3]
                  Survey.data1$COW_2C[UNKwhere] <- Survey.data$COW_2C[
                    UNKwhere
                  ] +
                    pred.unk1[, 4]
                  Survey.data1$LONE_COW[UNKwhere] <- Survey.data$LONE_COW[
                    UNKwhere
                  ] +
                    pred.unk1[, 5]
                  Survey.data1$TOT_CALVES[UNKwhere] <- Survey.data$TOT_CALVES[
                    UNKwhere
                  ] +
                    pred.unk1[, 6]
                } else {
              ...
              all_ratios_list$Total.pred[, b] <- c(
                Survey.data1$MOOSE_TOTA,
                rowSums(pred.numbers)
              )
              all_ratios_list$Total_LB[, b] <- c(
                Survey.data1$BULL_LARGE,
                pred.numbers$BULL_LARGE
              )
              all_ratios_list$Total_Calves[, b] <- c(
                Survey.data1$TOT_CALVES,
                pred.numbers$TOT_CALVES
              )
              all_ratios_list$Total_Cows[, b] <- c(
                Survey.data1$COW_1C +
                  Survey.data1$COW_2C +
                  Survey.data1$LONE_COW,
                pred.numbers$COW_1C +
                  pred.numbers$COW_2C +
                  pred.numbers$LONE_COW
              )
              all_ratios_list$Total_SB[, b] <- c(
                Survey.data1$BULL_SMALL,
              ...
              all_ratios_list$Total_Yrling_Cows[, b] <- pmin(
                all_ratios_list$Total_SB[, b],
                all_ratios_list$Total_Cows[, b]
              )
              # all_ratios_list$Total_Yrlings[,b] <- 2 * all_ratios_list$Total_SB[,b]
              all_ratios_list$Total_Yrlings[,
                b
              ] <- all_ratios_list$Total_Yrling_Cows[, b] +
                all_ratios_list$Total_SB[, b]
              ...
              all_ratios_list$Total_Mature_Cows[, b] <-
                all_ratios_list$Total_Cows[, b] -
                all_ratios_list$Total_Yrling_Cows[, b]
              all_ratios_list$Total_1C[, b] <- c(
                Survey.data1$COW_1C,
                pred.numbers$COW_1C
              )
              all_ratios_list$Total_2C[, b] <- c(
                Survey.data1$COW_2C,
                pred.numbers$COW_2C
              )
              all_ratios_list$Total_Bulls[, b] <- c(
                Survey.data1$BULL_LARGE + Survey.data1$BULL_SMALL,
  ...
  out <- list(
    total_model_id = total_model_id0,
...
mc_summarize_composition <- function(all_ratios_list) {
  ...
  dc <- lapply(all_ratios_list, function(z) {
    t(apply(z, 1, function(zz) {
      c(
        Mean = mean(zz),
        stats::quantile(zz, c(0.5, alpha / 2, (1 - alpha / 2)))
      )
    }))
  })
  dcell <- do.call(cbind, dc)
  colnames(dcell) <- paste(
    rep(names(dc), each = ncol(dc[[1]])),
    colnames(dc[[1]]),
    sep = "_"
  )
  ...
  dtot <- t(apply(d, 2, function(z) {
    c(Mean = mean(z), stats::quantile(z, c(0.5, alpha / 2, (1 - alpha / 2))))
  }))
...
subset_CPI_data <- function(CPI, ss) {
  ...
  if (nrow(CPIout$data) < opts$MINCELL) {
    stop(paste0(
      "Composition PIs cannot be provided for <",
      opts$MINCELL,
      " cells"
    ))
  }

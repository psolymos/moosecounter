...
zeroinfl2 <- function(
    ...
    .model_offset_2 <- function(x, terms = NULL, offset = TRUE) {
        ...
        if (length(offsets) > 0) {
            ...
            for (i in offsets) {
                ans <- ans + x[[deparse(attr(terms, "variables")[[i + 1]])]]
            }
            ans
        } else {
            ans <- if (offset) {
    ...
    if (hurdle) {
        ...
        zeroNegBin <- function(parms) {
            ...
            loglik0 <- suppressWarnings(dnbinom(
                0,
                size = theta,
                mu = mu,
                log = TRUE
        ...
        countNegBin <- function(parms) {
            ...
            loglik1 <- suppressWarnings(dnbinom(
                Y[Y1],
                size = theta,
                mu = mu,
                log = TRUE
        ...
        zeroBinom <- function(parms) {
            ...
            loglik <- sum(weights[Y0] * log(1 - mu[Y0])) +
                sum(
                    weights[Y1] *
                        log(mu[Y1])
                )
        ...
        gradfun <- function(parms) {
            c(
                countGrad(parms[1:(kx + (dist == "negbin"))]),
                zeroGrad(parms[
                    (kx + (dist == "negbin") + 1):(kx +
                        kz +
                        (dist == "negbin") +
        ...
        if (control$trace) {
            cat(
                ...
                paste(
                    ...
                    zero.dist,
                    "with",
                    ifelse(zero.dist == "binomial", linkstr, "log"),
                    "link\n"
                ),
                sep = ""
            )
        ...
        if (inherits(try(terms(ffz), silent = TRUE), "try-error")) {
            ffz <- eval(parse(
                text = sprintf(paste("%s -", deparse(ffc[[2]])), deparse(ffz))
            ))
        }
        ...
        if (
            zero.dist == "negbin" &
                isTRUE(all.equal(as.vector(Z), rep.int(Z[1], length(Z))))
        ) {
            stop(
                "negative binomial zero hurdle model is not identified with only an intercept"
            )
        }
        if (control$trace) {
        ...
        if (!is.null(start)) {
            ...
            if (length(start$count) != kx) {
                valid <- FALSE
                warning(
                    "invalid starting values, wrong number of count model coefficients"
                )
            }
            if (length(start$zero) != kz) {
                valid <- FALSE
                warning(
                    "invalid starting values, wrong number of zero-inflation model coefficients"
                )
            }
        ...
        if (is.null(start)) {
            ...
            model_zero <- switch(
                ...
                geometric = suppressWarnings(glm.fit(
                    Z,
                    factor(Y > 0),
                    family = binomial(),
                    weights = weights,
                ...
                binomial = suppressWarnings(glm.fit(
                    Z,
                    factor(Y > 0),
        ...
        if (separate) {
            ...
            theta <- c(
                count = if (dist == "negbin") {
                    as.vector(exp(fit_count$par[
                        kx +
                            1
                    ]))
                } else {
                ...
                zero = if (zero.dist == "negbin") {
                    as.vector(exp(fit_zero$par[
                        kz +
                            1
                    ]))
                } else {
            ...
            SE.logtheta <- list()
            if (dist == "negbin") {
                SE.logtheta$count <- as.vector(sqrt(diag(vc_count)[
                    kx +
                        1
                ]))
            ...
            if (zero.dist == "negbin") {
                SE.logtheta$zero <- as.vector(sqrt(diag(vc_zero)[
                    kz +
                        1
                ]))
            ...
            vc <- rbind(
                cbind(vc_count, matrix(0, kx, kz)),
                cbind(matrix(0, kz, kx), vc_zero)
            )
        ...
        rval <- list(
            ...
            control = control,
            start = start,
            weights = if (identical(as.vector(weights), rep.int(1L, n))) {
                NULL
            } else {
                weights
            },
            offset = list(
                count = if (identical(offsetx, rep.int(0, n))) {
                    NULL
                } else {
                    offsetx
                },
            ...
            df.null = nobs -
                2,
            df.residual = nobs -
                (kx + kz + (dist == "negbin") + (zero.dist == "negbin")),
            terms = list(count = mtX, zero = mtZ, full = mt),
            theta = theta,
            SE.logtheta = SE.logtheta,
            loglik = if (separate) {
                fit_count$value + fit_zero$value
            } else {
                fit$value
            },
            vcov = vc,
            ...
            separate = separate,
            converged = if (separate) {
                fit_count$convergence < 1 & fit_zero$convergence < 1
            } else {
                fit$convergence < 1
            },
            call = cl,
        ...
        ziPoissonNonZI <- function(parms) {
            ...
            #     )
            # loglik
            loglik <- stats::dpois(Y, lambda = mu, log = TRUE)
            sum(weights * loglik)
        }
        gradPoissonNonZI <- function(parms) {
            ...
            wres_zero <- ifelse(
                Y1,
                -1 / (1 - muz) * linkobj$mu.eta(etaz),
                (linkobj$mu.eta(etaz) - exp(clogdens0) * linkobj$mu.eta(etaz)) /
                    dens0
            )
        ...
        if (is.null(start)) {
            ...
            if (NonZI) {
                ...
                model_zero <- stats::glm.fit(
                    Z,
                    as.integer(Y0),
                    weights = weights,
                    family = stats::binomial(link = linkstr),
                    offset = offsetz
                )
                start <- list(
                    count = model_count$coefficients,
        ...
        if (!x$converged) {
            ...
            if (
                getOption("show.signif.stars") &
                    any(x$coefficients$count[, 4] < 0.1)
            ) {
                cat(
                    "---\nSignif. codes: ",
...
.zeroinfl_clpl <- function(
    ...
    nll_ZIP_ML <- function(parms) {
        ...
        loglik0 <- log(phi + exp(log(1 - phi) - mu))
        loglik1 <- log(1 - phi) + stats::dpois(Y, lambda = mu, log = TRUE)
        loglik <- sum(weights[id0] * loglik0[id0]) +
            sum(
                weights[id1] *
                    loglik1[id1]
            )
    ...
    nll_NB_ML <- function(parms) {
        mu <- as.vector(linkinvx(X %*% parms[1:kx] + offsetx))
        theta <- exp(parms[kx + 1])
        d <- suppressWarnings(stats::dnbinom(
            Y,
            size = theta,
            mu = mu,
            log = TRUE
    ...
    nll_ZINB_ML <- function(parms) {
        mu <- as.vector(linkinvx(X %*% parms[1:kx] + offsetx))
        theta <- exp(parms[kx + 1])
        phi <- as.vector(linkinvz(
            Z %*% parms[(kx + 1 + 1):(kx + kz + 1)] + offsetz
        ))
        loglik0 <- log(
            phi +
                exp(
    ...
    nll_ZINB_CL <- function(parms) {
        mu1 <- as.vector(linkinvx(X1 %*% parms[1:kx] + offsetx1))
        theta <- exp(parms[kx + 1])
        num <- suppressWarnings(stats::dnbinom(
            Y1,
            size = theta,
            mu = mu1,
            log = TRUE
        ...
        den <- log(
            1 -
                exp(suppressWarnings(stats::dnbinom(
                    0,
                    size = theta,
                    mu = mu1,
                    log = TRUE
    ...
    nll_CL <- switch(distr, "poisson" = nll_ZIP_CL, "negbin" = nll_ZINB_CL)
    res_CL <- suppressWarnings(stats::optim(
        inits[1:(np - kz)],
        nll_CL,
        method = method,
        hessian = FALSE,
    ...
    if (!zi) {
        nll_ML <- switch(distr, "poisson" = nll_P_ML, "negbin" = nll_NB_ML)
    ...
    ## logd0 needs to incorporate offsets but not weights
    logd0_fun <- switch(distr, "poisson" = logd0_ZIP, "negbin" = logd0_ZINB)
    logd0 <- logd0_fun(res_CL$par)

    nll_PL <- switch(distr, "poisson" = nll_ZIP_PL, "negbin" = nll_ZINB_PL)
    res_PL <- suppressWarnings(stats::optim(
        inits[(np - kz + 1):np],
        nll_PL,
        logd0 = logd0,
        method = method,
    ...
    ## CL+PL
    nll_ML <- switch(distr, "poisson" = nll_ZIP_ML, "negbin" = nll_ZINB_ML)

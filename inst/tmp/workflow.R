...
vars <- c(
    ...
    "NALC_Shrub",
    "Subalp_Shrub_250buf",
    "ELCSub_Fire8212DEM815",
    "SubShrub250_Fire8212DEM815"
)
...
data1 <- read.csv(file.path(
    "~/repos/DeducerPlugInMoose/inst/extdata",
...
ModelList[['TK_AlNeedWet_w']] <- wzi(
    zeroinfl(
        MOOSE_TOTA ~ TKStrat_01 + NALC_AllNeedleWet | 1,
...
ModelList[['TK_AlNeedWet_w']] <- wzi(
    zeroinfl(
        MOOSE_TOTA ~ LKStrat_01 + AllNeedleWet | 1,
        data = MooseData[MooseData$srv, ],
        dist = 'negbin',
        link = 'logit'
    )
...
mP2 <- glm(
    MOOSE_TOTA ~ TKStrat_01 + NALC_AllNeedleWet,
    data = MooseData[MooseData$srv, ],
    family = poisson
)
mNB <- zeroinfl2(
...
ML[["A"]] <- mc_fit_total(
    x,
    c("MMU_ID", "AllNeedleWet", "LKStrat_01", "DEM800to1200m"),
    dist = "NB",
    weighted = FALSE
)
ML[["B"]] <- mc_fit_total(
...
CML[["A"]] <- mc_fit_comp(
    x,
    vars = c("DEM800to1200m", "Fire1982to2012", "NALC_DesShrMix")
)
...
library(moosecounter)
x <- read.csv(
    "~/Dropbox/a8m/projects-2022/yt-000-moosecounter/Cassiar2020_QuerriedSUs_ForFinalAnalysis.csv"
)
switch_response("total")
# x$UNKNOWN_AG[] <- 0
x <- mc_update_total(x)
